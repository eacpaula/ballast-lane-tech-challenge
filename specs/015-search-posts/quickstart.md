...
5. Validate authenticated search:
   ...
   - if needed, add one owned private matching post and one other-user private
     matching post to validate visibility boundaries
...
For private-visibility validation, it is acceptable to insert temporary local
demo fixtures directly into the Compose-backed PostgreSQL database and reset
back to seed state afterward.
