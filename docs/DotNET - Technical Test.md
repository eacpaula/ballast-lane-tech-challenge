# .NET - Technical Interview Test

## Project Overview

Your task is to develop a simple web application with an API and data layer using .NET C#,
[ASP.NET](http://ASP.NET) MVC, Web API, and a database or data store, while adhering to Clean Architecture
...
● **API**:
○ Develop an [ASP.NET](http://ASP.NET) Web API with endpoints that allow users to perform CRUD
...
● Each task has a title, description, status, and due\_date
...
and you will screen share either your GitHub repository or IDE.
.NET - Technical Interview Exercise
